...
# SYNOPSIS
#
#   WDB_ENABLE(variable-prefix, option-name, pretty-name, default)
#   WDB_IF_ENABLED(option-name, action-if-enabled, action-if-disabled)
#   WDB_IF_ENABLED_ANY(option-names, action-if-enabled, action-if-disabled)
#   WDB_IF_ENABLED_ALL(option-names, action-if-enabled, action-if-disabled)
#   WDB_ENABLE_CONDITIONAL(variable-prefix, option-name)
#
# DESCRIPTION
#
#   The WDB_ENABLE macro creates a --enable-<option-name>
...
#   command-line option.
#
#   The WDB_IF_ENABLED macro executes <action-if-enabled> if the
#   value of the enabled_<option-name> shell variable is "yes", and
#   <action-if-disabled> otherwise.  Either or both actions can be
#   empty.
#
#   The WDB_IF_ENABLED_ANY macro executes <action-if-enabled> if the
...
#
#   The WDB_IF_ENABLED_ALL macro executes <action-if-enabled> if the
#   enabled_<option> shell variable is "yes" for all of the specified
#   options, and <action-if-disabled> otherwise.  Either or both
#   actions can be empty.
#
#   The WDB_ENABLE_CONDITIONAL macro defines an automake conditional
...
#
#   The option names passed to the WDB_IF_* macros can be
#   expressions that are evaluated at runtime (e.g. shell variables or
#   backtick expressions).
#
# NOTES
#
#   The extra null statements in the WDB_IF_* macros are there in
...
# $3 = action-if-disabled
#
AC_DEFUN([WDB_IF_ENABLED_ANY], [
    wdb_if_enabled_any_result=no
    for component in $1 ; do
	if eval "test x\"\${enable_$component}\" = x\"yes\"" ; then
	    wdb_if_enabled_any_result=yes;
	    break;
	fi
    done
    if test x"$wdb_if_enabled_any_result" = x"yes" ; then
	:; $2
    else
        :; $3
    fi
    unset wdb_if_enabled_any_result
])
...
# $3 = action-if-disabled
#
AC_DEFUN([WDB_IF_ENABLED_ALL], [
    wdb_if_enabled_all_result=yes
    for component in $1 ; do
	if eval "test x\"\${enable_$component}\" != x\"yes\"" ; then
	    wdb_if_enabled_all_result=no;
	    break;
	fi
    done
    if test x"$wdb_if_enabled_all_result" = x"yes" ; then
	:; $2
    else
        :; $3
    fi
    unset wdb_if_enabled_all_result
])

#
# WDB_ENABLE_CONDITIONAL
#
# $1 = variable-prefix
# $2 = option-name
#
AC_DEFUN([WDB_ENABLE_CONDITIONAL], [
